...
def run_benchmark(model_name, tp, dp, qps, max_tokens=3, max_batch_tokens=512, max_num_seqs=512):
    """Run single benchmark configuration"""
    output_dir = f"./vllm_output/vidur_qps/{model_name.replace('/', '_')}/tp{tp}_dp{dp}/qps{qps}"
...
def main():
    ...
    parser.add_argument("--csv-path", default="qps_by_model_parallelism_device.csv", help="Path to QPS CSV file")
    ...
    for (tp, replicas), qps_list in qps_configs.items():
        dp = 1  # Data parallelism fixed to 1 for single node
        ...
        if tp * dp <= args.total_gpus:
            ...
            for qps in qps_list:
                try:
                    run_benchmark(args.model, tp, dp, qps)
